...
import { findUserById, login, signup } from '@/services/portal.service';
...
async function hashSessionToken(token: string): Promise<string> {
  const secret = env.PORTAL_SESSION_SECRET || 'portal-dev-secret-do-not-use-in-prod';
...
portalRoutes.post('/api/logout', (c) => {
  const token = getCookie(c, SESSION_COOKIE_NAME);
  if (token) {
    hashSessionToken(token).then((hash) => activeSessions.delete(hash));
  }
